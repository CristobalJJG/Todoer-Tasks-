...
def index():
    db, c = get_db()
    c.execute(
        """SELECT t.id, t.description, u.username,
        t.completed, t.created_at
        FROM todo t JOIN user u 
...
def create():
    if request.method == 'POST': 
        ...
        error = None
        if not description: error='Descripción requerida.'
        if error is not None: 
            ...
            bd.commit()
            consola("Creado el todo: " + str(g.user['id']) + ", Descripción: " + description)
...
def get_todo(id):
    ...
    c.execute(
        ...
        t.created_by, t.created_at, u.username
        FROM todo t JOIN user u 
        ON t.created_by = u.id 
        where t.id = %s;"""
        ,(id,)
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db.commit()
            return redirect(url_for("todo.index"))
    consola("Editado el todo: " + str(id) + ", User:" + str(g.user['id']))
...
def delete(id):
    db, c = get_db()    
    consola("Eliminado el todo: " + str(id) + ", User:" + str(g.user['id']))
